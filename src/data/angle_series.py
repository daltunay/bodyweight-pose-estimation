...
class AngleSeries:
    ...
    def __init__(
        ...
        Args:
            * coordinates (CoordinateSeries): Coordinates series from which the angles\
                will be extracted

            * angle_list (Iterable[AngleJoints]): List of the angles to extract from\
                the series
        """
        ...
        self.frame.loc["orientation"] = self.extract_orientation(
            row_1="MID_ANKLE", row_2="NOSE"
        )

    def extract_angles(self) -> pd.DataFrame:
        """Method to extract the angle values from the (x, y, z) coordinates\
            of each joint
    ...
    def extract_orientation(self, row_1, row_2):
        orientations = []
        a_series = self.coordinates.frame.loc[row_1]
        b_series = self.coordinates.frame.loc[row_2]

        # computing each video frame
        for i in range(self.coordinates.frame.shape[1]):
            a = a_series.values[i]
            b = b_series.values[i]
    ...
    def scale(
        ...
        orientation = self.frame.loc["orientation"]
        ...
        scaled_frame.loc["orientation"] = orientation
    ...
    def plot(
        ...
        Args:
            * angles (Iterable[AngleJoints], optional): List of angles for which to\
                plot the angle value evolution. Defaults to None

            * figsize (Tuple, optional): Figure size. Defaults to (24, 10).

            * title (str, optional): Plot title. Defaults to "Angle coordinates\
                evolution over time"
